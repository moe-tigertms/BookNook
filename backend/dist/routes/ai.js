    ...
    try {
        ...
        for (const b of byAuthor) {
            if (!seen.has(b.id)) {
                ...
                combined.push({ ...rest, isCheckedOut: b.checkouts.length > 0, reason: `Same author: ${book.author}` });
            }
        }
        for (const b of byGenre) {
            if (!seen.has(b.id) && combined.length < limit) {
                ...
                combined.push({ ...rest, isCheckedOut: b.checkouts.length > 0, reason: `Same genre: ${book.genre}` });
            }
        }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return res.status(502).json({
                ...
                fallback: book.description?.slice(0, 300) ?? `"${book.title}" by ${book.author}.`,
                details: err,
            });
        }
